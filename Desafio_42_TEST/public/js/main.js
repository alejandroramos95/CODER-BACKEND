...
function enableChat(){
    ...
    document.getElementById('content-flex').innerHTML +='<button type="button" class="btn btn-danger" id="deleteUser">Eliminar cuenta</button>';
    document.getElementById('deleteUser').addEventListener('click', ConfirmDeleteUser);
...
async function createUser(){
    ...
    await socket.on('response-checkEmail', async(response) =>{
        if(response){
            ...
            if (await sendRegisterForm(author))
                window.location.href = "/";
            else{
                window.location.href = "/fail_register";
            }
        }else{
...
async function ConfirmDeleteUser(){
    if(confirm('¿Está seguro que desea eliminar su cuenta?')){
        await DeleteUser()
        ? Logout()
        : alert('Ha ocurrido un error al intentar eliminar su cuenta.');
    }
}
